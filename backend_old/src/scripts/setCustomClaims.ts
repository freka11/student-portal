 ...
 * Usage: npx ts-node src/scripts/setCustomClaims.ts
 */
...
// Initialize Firebase Admin using existing configuration
...
async function main() {
  ...
  switch (command) {
    case 'set-admin':
      ...
      if (!adminUid) {
        console.log('Usage: npx ts-node src/scripts/setCustomClaims.ts set-admin <uid>')
        return
      }
    ...
    case 'set-student':
      ...
      if (!studentUid) {
        console.log('Usage: npx ts-node src/scripts/setCustomClaims.ts set-student <uid>')
        return
      }
    ...
    case 'set-teacher':
      ...
      if (!teacherUid) {
        console.log('Usage: npx ts-node src/scripts/setCustomClaims.ts set-teacher <uid>')
        return
      }
    ...
    case 'get':
      const getUid = args[1]
      if (!getUid) {
        console.log('Usage: npx ts-node src/scripts/setCustomClaims.ts get <uid>')
        return
      }
...
Usage:
  npx ts-node src/scripts/setCustomClaims.ts <command> [uid]
