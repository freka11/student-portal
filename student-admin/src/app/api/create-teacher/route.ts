...
export async function POST(request: NextRequest) {
  try {
    ...
    if (counterDoc.exists) {
      ...
      await adminFirestore.collection('counters').doc('teacherCounter').update({
        value: counterValue
      })
    } else {
      await adminFirestore.collection('counters').doc('teacherCounter').set({
        value: counterValue
      })
    }
